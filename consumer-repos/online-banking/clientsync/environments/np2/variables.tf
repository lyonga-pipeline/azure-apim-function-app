variable "subscription_id" {
  type        = string
  description = "NP2 workload subscription ID."
}
...
variable "location" {
  type        = string
  description = "Azure region for ClientSync np2 resources."
}
...
variable "environment" {
  ...
  validation {
    condition     = var.environment == "np2"
    error_message = "This root is only for the ClientSync np2 environment."
  }
}
