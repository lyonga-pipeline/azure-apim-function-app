...
run "secure_defaults" {
  ...
  assert {
    condition     = azurerm_windows_web_app_slot.this.https_only == true
    error_message = "https_only should default true"
  }
  assert {
    condition     = azurerm_windows_web_app_slot.this.public_network_access_enabled == false
...
run "with_stack_and_identity" {
  ...
  assert {
    condition     = one(azurerm_windows_web_app_slot.this.site_config).always_on == true
    error_message = "site_config.always_on not wired"
  }
  assert {
    condition     = length(azurerm_windows_web_app_slot.this.identity) == 1
