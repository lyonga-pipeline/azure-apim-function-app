variable "name" {
  ...
  validation {
    ...
    error_message = "Key Vault names must be 3-24 characters, start with a letter, end with a letter or number, and contain no consecutive hyphens."
  }
}
...
variable "purge_protection_enabled" {
  description = "Purge protection. Once enabled on a vault it cannot be disabled."
  type        = bool
  default     = true
}

variable "public_network_access_enabled" {
  description = "Whether the vault is reachable from public networks. Defaults closed; use the private-endpoint module."
  type        = bool
  default     = false
}
...
variable "access_policies" {
  description = "Access policies as a list (backward-compatible). Used only when rbac_authorization_enabled = false. Prefer access_policies_by_key for stable identity."
  type = list(object({
    tenant_id               = string
    object_id               = string
...
variable "access_policies_by_key" {
  description = "Access policies keyed by stable business identifier (e.g. \"group-reader\"). Used only when rbac_authorization_enabled = false."
  type = map(object({
    tenant_id               = string
    object_id               = string
    application_id          = optional(string)
...
variable "network_acls" {
  description = "Network ACLs. null omits the block; {} applies deny-by-default (bypass AzureServices)."
  type = object({
...
variable "contacts" {
  description = "Certificate contacts as a list (backward-compatible)."
  type = list(object({
    email = string
