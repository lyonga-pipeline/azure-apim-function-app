...
output "resource_group_name" {
  ...
  value       = azurerm_network_interface.interface.resource_group_name
}

output "mac_address" {
  description = "MAC address assigned to the NIC (known after apply)."
  value       = azurerm_network_interface.interface.mac_address
}
...
output "private_ip_addresses" {
  ...
  value       = azurerm_network_interface.interface.private_ip_addresses
}

output "ip_configurations" {
  description = "IP configuration attributes keyed by config name."
  value = {
    for cfg in azurerm_network_interface.interface.ip_configuration : cfg.name => {
      name                          = cfg.name
