output "ids" {
  ...
  value       = { for key, gateway in azurerm_local_network_gateway.gateway : key => gateway.id }
}

output "names" {
  description = "Local network gateway names keyed by input key."
  value       = { for key, gateway in azurerm_local_network_gateway.gateway : key => gateway.name }
}

output "gateway_addresses" {
  description = "Local network gateway public addresses keyed by input key."
  value       = { for key, gateway in azurerm_local_network_gateway.gateway : key => gateway.gateway_address }
}

output "gateways" {
  description = "Local network gateway attributes keyed by input key for downstream composition."
  value = {
    for key, gateway in azurerm_local_network_gateway.gateway : key => {
      id                  = gateway.id
      name                = gateway.name
