resource "azurerm_eventgrid_event_subscription" "subscription" {
  ...
  scope                         = azurerm_eventgrid_namespace.main.id
  ...
  dynamic "subject_filter" {
    for_each = each.value.subscription.subject_filter != null ? [1] : []
    content {
      subject_begins_with = lookup(each.value.subscription.subject_filter, "subject_begins_with", null)
      subject_ends_with   = lookup(each.value.subscription.subject_filter, "subject_ends_with", null)
      case_sensitive      = lookup(each.value.subscription.subject_filter, "case_sensitive", false)
    }
  }

  dynamic "advanced_filter" {
    for_each = each.value.subscription.advanced_filter != null ? each.value.subscription.advanced_filter : []
    content {
      string_in {
        key    = lookup(advanced_filter.value, "key", null)
        values = lookup(advanced_filter.value, "values", [])
      }
    }
  }
}