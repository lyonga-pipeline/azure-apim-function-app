output "ids" {
  ...
  value       = { for key, value in azurerm_private_dns_zone.zone : key => value.id }
}
output "names" {
  description = "Map of caller-supplied key to private DNS zone name."
  value       = { for key, value in azurerm_private_dns_zone.zone : key => value.name }
}
output "resource_group_names" {
  description = "Map of caller-supplied key to the resource group name hosting each zone."
  value       = { for key, value in azurerm_private_dns_zone.zone : key => value.resource_group_name }
}
output "zones" {
  ...
  value = {
    for key, value in azurerm_private_dns_zone.zone : key => {
      id                  = value.id
      name                = value.name
