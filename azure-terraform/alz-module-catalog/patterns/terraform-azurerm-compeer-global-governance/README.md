...
The Azure Policy required tag names are aligned with `terraform-azurerm-compeer-platform-tags`' `mandatory_keys` (its Phase 7 rebuild): `environment`, `application`, `appcode`, `owner`, `source_repo`, `created_on`, `criticality_tier`, `data_classification`, `lifecycle_state`, `cost_center`, `gl_category`, and `created_by`. The tag module automatically emits `created_by = "Terraform"`. (This repo doesn't contain the OPA policy-code repo referenced by `azure-pipelines-opa-policy-code.yml` — verify its tag data file separately if it also encodes this vocabulary.)
