output "zone_ids" {
  ...
  value       = { for key, zone in cloudflare_zone.zone : key => zone.id }
}

output "record_ids" {
  description = "Cloudflare record IDs keyed by input key."
  value       = { for key, record in cloudflare_record.record : key => record.id }
}

output "ruleset_ids" {
  description = "Cloudflare ruleset IDs keyed by input key."
  value       = { for key, ruleset in cloudflare_ruleset.ruleset : key => ruleset.id }
}
