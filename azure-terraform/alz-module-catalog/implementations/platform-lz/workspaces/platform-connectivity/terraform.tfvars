...
platform_tags = {
  ...
  # (connectivity -> conn), so the tag matches the actual name prefix.
  appcode     = "conn"
  owner       = "Cloud Enablement"
  source_repo = "ado://Compeer/landing-zone"
  created_on  = "2026-01-01"
  # Platform tier-0: foundational enterprise/platform service (hub
  # networking) required for other systems.
  ...
  # bronze/none) and would fail the new validation. Read as "silver" - hub
  # networking needing formal DR with less aggressive RTO/RPO than a
  # member-facing system - but this is my inference, not a confirmed
  # decision; get this confirmed with whoever owns the actual DR posture.
  dr_tier = "silver"
