...
resource "terraform_data" "contract_versions" {
  input = {
    ...
    connectivity_contract_version = try(local.connectivity_outputs.contract_version, null)
  ...
  lifecycle {
    ...
    precondition {
      condition     = try(local.connectivity_outputs.contract_version, null) == null || try(local.connectivity_outputs.contract_version, null) == "0.1.0"
      error_message = "platform-connectivity's published contract_version has changed - review its outputs.tf for breaking changes before updating the expected version in this file."
    }
    precondition {
